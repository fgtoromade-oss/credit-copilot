...
# DATA_DIR points at the mounted persistent disk on Render (set via render.yaml).
# Defaults to the current directory for local development, where it behaves
# exactly as before.
DATA_DIR = os.environ.get("DATA_DIR", ".")
app.config["UPLOAD_FOLDER"] = os.path.join(DATA_DIR, "uploads")
app.config["MAX_CONTENT_LENGTH"] = 50 * 1024 * 1024
DB_PATH = os.path.join(DATA_DIR, "data", "credit_copilot.db")
...
MAX_EXTRACT_CHARS = 15000  # keeps prompts within a safe token budget per document

def extract_text_from_document(filepath, filename):
    """Best-effort text extraction so document contents can be fed to the AI.
    Returns a string (possibly truncated), or an explanatory placeholder if
    the file type can't be read."""
    ext = filename.rsplit(".", 1)[-1].lower() if "." in filename else ""
    try:
        if ext == "pdf":
            if not PDFPLUMBER_AVAILABLE:
                return "[Could not extract: pdfplumber not installed]"
            text_parts = []
            ...
            if not OPENPYXL_AVAILABLE:
                return "[Could not extract: openpyxl not installed]"
            ...
            if not DOCX_AVAILABLE:
                return "[Could not extract: python-docx not installed]"
            ...
            return f"[No text extraction available for .{ext} files]"
...
class PGConnection:
    """Thin wrapper so existing conn.execute(query, params) call sites work
    unchanged against psycopg2, translating SQLite's '?' placeholders to
    Postgres's '%s' and returning dict-like rows (row['col']) like sqlite3.Row."""
...
# All page templates use {% extends "base.html" %}, but this app has no
# templates/ folder on disk (everything is rendered via render_template_string).
# DictLoader lets Jinja resolve "base.html" to the BASE_TEMPLATE string above.
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body d-flex flex-wrap gap-2 align-items-center">
                                        <strong class="me-2"><i class="bi bi-stars"></i> AI Generation</strong>
                                        <form method="POST" action="{{ url_for('generate_financial_analysis', deal_id=deal.id) }}" class="d-inline">
                                            <button type="submit" class="btn btn-accent btn-sm">
                                                <i class="bi bi-magic"></i> Generate Financial Statement Analysis
                                            </button>
                                        </form>
                                        <form method="POST" action="{{ url_for('generate_bank_analysis', deal_id=deal.id) }}" class="d-inline">
                                            <button type="submit" class="btn btn-accent btn-sm">
                                                <i class="bi bi-magic"></i> Generate Bank Statement Analysis
                                            </button>
                                        </form>
                                        <small class="text-muted ms-auto">Uses documents uploaded in the Documents tab</small>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    ...
                                    <div class="d-flex gap-2">
                                        <form method="POST" action="{{ url_for('generate_memo', deal_id=deal.id) }}" class="d-inline" onsubmit="return confirm('This will replace the current memo content with an AI-drafted version. Continue?');">
...
def deal_detail(deal_id):
    ...
    if deal is None:
        conn.close()
        flash("That deal could not be found. It may have been deleted, or the database was reset on the last deploy.", "error")
...
def gemini_status_error():
    """Returns a user-facing message if Gemini isn't usable, else None."""
    if not GEMINI_AVAILABLE:
        return "The google-generativeai package isn't installed on the server."
    if not os.environ.get("GEMINI_API_KEY"):
        return "No GEMINI_API_KEY is set in the environment. Add it in Render's environment variables."
    return None

@app.route("/deal/<deal_id>/financial/generate", methods=["POST"])
def generate_financial_analysis(deal_id):
    err = gemini_status_error()
    if err:
        flash(err, "error")
        return redirect(url_for("deal_detail", deal_id=deal_id))
    ...
    deal = conn.execute("SELECT * FROM deals WHERE id = ?", (deal_id,)).fetchone()
    docs = conn.execute(
        "SELECT * FROM documents WHERE deal_id = ? AND doc_type IN ('financial_statements','management_accounts')",
        (deal_id,)
    ).fetchall()

    if not docs:
        conn.close()
        flash("Upload a Financial Statement or Management Accounts document first, then generate analysis.", "error")
        return redirect(url_for("deal_detail", deal_id=deal_id))

    doc_text = ""
    for doc in docs:
        doc_text += f"\n\n=== Document: {doc['filename']} ===\n"
        doc_text += extract_text_from_document(doc["filepath"], doc["filename"])

    context = f"Company: {deal['name']}, Industry: {deal['industry']}, Requested facility: NGN {deal['loan_amount']}"
    prompt = (
        "Analyze the financial statement data below for a commercial lending credit assessment. "
        "Extract key figures where visible (revenue, net profit, total assets, total liabilities, "
        "current assets, current liabilities, equity, EBITDA, interest expense, existing debt). "
        "Calculate standard ratios where the underlying figures are available (current ratio, "
        "debt/equity, net margin, ROE, interest coverage). Then write a concise professional "
        "commentary covering liquidity, leverage, profitability, and repayment capacity, in the "
        "style of a commercial bank credit memo. If a figure isn't available in the source "
    ...
    if result is None:
        flash("AI analysis failed to generate a response. Check server logs.", "error")
        return redirect(url_for("deal_detail", deal_id=deal_id))
    if result.startswith("[AI temporarily unavailable"):
        flash(result, "error")
        return redirect(url_for("deal_detail", deal_id=deal_id))
    ...
    conn.execute("""
        ...
        VALUES (?, ?, ?, ?, ?)
    """, (analysis_id, deal_id, "financial_statements", result, datetime.datetime.now().isoformat()))
    conn.commit()
    conn.close()
    flash("AI financial statement analysis generated!", "success")
    return redirect(url_for("deal_detail", deal_id=deal_id))

@app.route("/deal/<deal_id>/bank/generate", methods=["POST"])
def generate_bank_analysis(deal_id):
    err = gemini_status_error()
    if err:
        flash(err, "error")
        return redirect(url_for("deal_detail", deal_id=deal_id))
    ...
    deal = conn.execute("SELECT * FROM deals WHERE id = ?", (deal_id,)).fetchone()
    docs = conn.execute(
        "SELECT * FROM documents WHERE deal_id = ? AND doc_type = 'bank_statements'",
        (deal_id,)
    ).fetchall()

    if not docs:
        conn.close()
        flash("Upload a Bank Statement document first, then generate analysis.", "error")
        return redirect(url_for("deal_detail", deal_id=deal_id))
    ...
    context = f"Company: {deal['name']}, Industry: {deal['industry']}, Requested facility: NGN {deal['loan_amount']}"
    prompt = (
        "Analyze the bank statement transaction data below for a commercial lending credit "
        "assessment. Estimate, where the data allows: average monthly turnover, average balance, "
        "the largest single inflow, the largest single outflow, cash flow trend over the period, "
        "any visible seasonality, and any recurring existing loan repayments. Then write a concise "
        "professional commentary on the customer's cash flow health and repayment capacity, in the "
        "style of a commercial bank credit memo. If the statement doesn't cover enough months to "
        "assess a trend, say so rather than guessing.\n\n"
        f"DOCUMENT CONTENT:{doc_text}"
    )
    result = get_gemini_response(prompt, context)
    conn.close()
    ...
    conn.execute("""
        INSERT INTO financial_analysis (id, deal_id, analysis_type, data, created_at)
        VALUES (?, ?, ?, ?, ?)
    """, (analysis_id, deal_id, "bank_statements", result, datetime.datetime.now().isoformat()))
    conn.commit()
    conn.close()
    flash("AI bank statement analysis generated!", "success")
    return redirect(url_for("deal_detail", deal_id=deal_id))

@app.route("/deal/<deal_id>/memo/generate", methods=["POST"])
def generate_memo(deal_id):
    ...
    financials = conn.execute(
        "SELECT * FROM financial_analysis WHERE deal_id = ? ORDER BY created_at DESC", (deal_id,)
    ).fetchall()
    meetings = conn.execute(
        "SELECT * FROM meetings WHERE deal_id = ? ORDER BY created_at DESC", (deal_id,)
    ).fetchall()
    conn.close()

    deal_summary = (
        f"Company Name: {deal['name']}\n"
        f"Industry: {deal['industry']}\n"
        f"Requested Facility Amount: NGN {deal['loan_amount']}\n"
        f"Loan Purpose: {deal['loan_purpose']}\n"
        f"Business Overview: {deal['business_overview'] or 'Not captured'}\n"
        f"Initial Risks Noted: {deal['initial_risks'] or 'Not captured'}\n"
        f"Recommended Facility Amount: {deal['facility_amount'] or 'Not yet recommended'}\n"
        f"Recommended Tenor: {deal['facility_tenor'] or 'Not yet recommended'}\n"
        f"Repayment Frequency: {deal['facility_repayment'] or 'Not yet recommended'}\n"
        f"Conditions: {deal['facility_conditions'] or 'None recorded'}\n"
        f"Covenants: {deal['facility_covenants'] or 'None recorded'}\n"
        f"Collateral: {deal['facility_collateral'] or 'None recorded'}\n"
        f"Facility Justification: {deal['facility_justification'] or 'Not yet recorded'}\n"
    )
    for m in meetings:
        deal_summary += f"\nMeeting Summary: {m['summary'] or m['notes'] or 'No summary captured'}\n"
    for f in financials:
        deal_summary += f"\n--- {f['analysis_type'].replace('_',' ').title()} (recorded {f['created_at'][:10]}) ---\n{f['data']}\n"

    prompt = (
        "Draft a complete commercial credit memorandum for the deal described below, written in "
        "the voice of an experienced commercial banker preparing a submission for a Management "
        "Credit Committee. Use clear numbered section headings covering: Borrower Details, "
        "Facility Requested, Business Overview, Financial Analysis, Bank Statement / Cash Flow "
        "Analysis, Risk Assessment, Facility Recommendation & Justification, and Conditions/"
        "Covenants/Collateral. Be factual and frank rather than promotional. Where information "
        "hasn't been captured yet, note it as outstanding rather than inventing figures.\n\n"
        f"DEAL INFORMATION:\n{deal_summary}"
    )
    result = get_gemini_response(prompt, f"Deal: {deal['name']}")

    if result is None:
        flash("AI memo generation failed to generate a response. Check server logs.", "error")
        return redirect(url_for("deal_detail", deal_id=deal_id))
    if result.startswith("[AI temporarily unavailable"):
        flash(result, "error")
        return redirect(url_for("deal_detail", deal_id=deal_id))
    ...
    conn.execute("UPDATE deals SET memo_content = ?, updated_at = ? WHERE id = ?",
                 (result, datetime.datetime.now().isoformat(), deal_id))
    conn.commit()
    conn.close()
    flash("AI-drafted credit memo generated! Review and edit before submission.", "success")
    return redirect(url_for("deal_detail", deal_id=deal_id))
